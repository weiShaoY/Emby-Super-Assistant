  ...
  duplicate() {
    ...
    function findDuplicatesByProperty(
      ...
      const propertyMap = list.reduce((acc, item) => {
        ...
        acc[key].push(item)
        return acc
      }, {} as { [key: string]: VideoType.Video[] })

      return Object.values(propertyMap).flat()
     ...
     */
    function getUniqueVideoProcessedName(items: VideoType.Video[]): string[] {
      const videoProcessedNameMap = items.reduce((acc, item) => {
        acc[item.videoProcessedName] = acc[item.videoProcessedName] || []
        acc[item.videoProcessedName].push(item)
        return acc
      }, {} as { [key: string]: VideoType.Video[] })

      return Object.keys(videoProcessedNameMap).filter(key => videoProcessedNameMap[key].length > 1)
    }
    ...
    function handleDuplicates(): {
      ...
      return {
         ...
         *  Emby去重的影片标题列表
         */
        duplicatesVideoNameList: getUniqueVideoProcessedName(videoFileArray),
      }
    }
